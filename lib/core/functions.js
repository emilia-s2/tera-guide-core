...
module.exports = (mod, guide, lang, dispatch, params) => {
	...
	// Messages colors
	global.cr = '</font><font color="#ff5cde">';  // verde claro
	...
	global.clp = '</font><font color="#8eff05">'; // rosa claro
	global.clb = '</font><font color="#00ffff">'; // light blue
	global.cbl = '</font><font color="#ff0000">'; // vermelho
	...
	global.loadHandler = (zone, debug_enabled = false) => {
		...
		try {
			...
			if (typeof object === "function") {
				...
				try {
					guide.context = object(dispatch, eventHandlers, guide, lang);
				} catch (e) {
					return mod.error(e);
				}
			} else {
...
/*		textHandler({
		...
		textHandler({
			"sub_type": "CYMSG",
			"delay": 5000,
			"message": `problemas com comando (guia gui)? encerra o proxy e delete o arquivo (config.json) em Guide-Complemento, se esta 100% ignora este aviso.`
